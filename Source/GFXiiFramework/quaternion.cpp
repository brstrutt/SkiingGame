...
void Quaternion::ConvertToRotationMatrix(Matrix4x4* rot) const
{
	if ( rot == NULL )
		return;
...
void Quaternion::SetQuatByVectors(Vector3 source, Vector3 dest)
{
	//get the angle between them
	float angle = acos(source.DotProduct(dest));
	//get the axis of rotation
	Vector3 axisOfRotation = source.CrossProduct(dest).Normalise();

	SetQuaternion(axisOfRotation * sin(angle / 2), cos(angle / 2));
}
